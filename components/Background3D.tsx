...
const DNAHelixOptimized = () => {
  ...
  // Reduced count for better performance on mobile/free tier hosting
  const count = 40; 
  const radius = 2.5;
  const height = 18;
...
const Background3D: React.FC = () => {
  return (
    <div className="fixed inset-0 z-0 bg-black">
      ...
      <Canvas camera={{ position: [0, 0, 14], fov: 40 }} dpr={[1, 1.25]} performance={{ min: 0.5 }}>
        <fog attach="fog" args={['#000000', 10, 30]} />
        <ambientLight intensity={0.5} />
        
        <DNAHelixOptimized />
        
        {/* Significantly reduced star count for memory optimization */}
