  ...
  useEffect(() => {
    ...
    if (SpeechRecognition) {
      ...
      recognition.onresult = (event) => {
        ...
        setSymptomText(prev => prev + finalTranscript + interimTranscript);
      };
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div>
        ...
        <div className="relative">
          <textarea id="symptoms" rows={5} className="w-full bg-slate-900 border border-slate-600 rounded-xl p-4 text-gray-200 focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 transition-colors placeholder-gray-500" placeholder="صف أعراضك هنا..." value={symptomText} onChange={(e) => setSymptomText(e.target.value)} />
      ...
      <div className="flex justify-center pt-4">
        <button onClick={onAnalyze} disabled={!hasContent} className="bg-cyan-600 text-white font-bold py-3 px-10 rounded-full transition-all duration-300 transform hover:scale-105 shadow-lg shadow-cyan-500/30 flex items-center justify-center gap-3 disabled:bg-slate-600 disabled:text-gray-400 disabled:cursor-not-allowed disabled:transform-none disabled:shadow-none">
          <SparklesIcon />
          حلل الآن
        </button>
      </div>
